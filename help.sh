...
_help_examples() {
    echo "Example startup:"
    echo "  # install dependencies and run tests in one shot:"
    echo "  make deps test"
    echo ""
    echo "  # in ONE shell, start the live-reloading static assets build:"
    echo "  make run_static"
    echo ""
    echo "  # in SECOND shell (separate from above), start live-reloading site build:"
    echo "  make run_site"
}
